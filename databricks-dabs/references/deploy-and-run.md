...
**Always validate after any configuration change**, then run `bundle summary` to confirm the fully-resolved configuration matches your intent:

- `bundle summary`
- `bundle summary -t prod`

`bundle validate` checks the configuration for errors; `bundle summary` shows the resolved values (variables, targets, paths) that will actually be deployed. Running both catches configuration issues that a single validate pass can miss.

> **Note:** The `--strict` flag is no longer used. Combine `bundle validate` with `bundle summary` for a thorough pre-deploy check.
...
- After fixing an error, validate the fix with the appropriate command:
  ...
  - `bundle validate` (followed by `bundle summary`) otherwise
...
| **Hardcoded catalog in dashboard** | Use dataset_catalog parameter (Databricks CLI v1.0.0+)                  |
...
| **Validation errors unclear**      | Re-run with `databricks bundle validate --debug`, then `databricks bundle summary` |
