...
**Status:** mixed catalog — 100+ managed connectors are available. GA connectors are production-ready; preview and beta connectors expand coverage for newer SaaS, ads, file-source, database, and community connector scenarios. See the connector catalog below.
...
3. **Query-based** — for sources without native CDC (Oracle / Teradata / SQL Server / PG / MySQL query-based, Snowflake / Redshift / Synapse / BigQuery via Foreign Catalog), the connector issues periodic queries instead of subscribing to a change feed. Supported destination modes include `SCD_TYPE_1`, `SCD_TYPE_2`, and `APPEND_ONLY`.
4. **File-source connectors** — managed ingestion from collaboration/document repositories such as SharePoint and Google Drive when those connectors are enabled.
5. **Community connectors** — open-source template-based connectors for custom sources, currently beta.
...
Lakeflow Connect ships 100+ connectors at multiple release stages. **GA** connectors are production-ready; preview and beta connectors may have feature gates, changing configuration, or support limitations. Always check the per-connector docs before promising production support.
...
6. **Schedule** the pipeline directly with one or more custom Lakeflow Connect schedules. Lakeflow Connect auto-creates a Databricks Job for each schedule; use external Jobs orchestration only for broader multi-task workflows.
...
**3. `continuous: true` is rejected — use pipeline schedules instead.**

```json
// WRONG: continuous mode fails at create
{"continuous": true, "ingestion_definition": {/* ... */}}
// RIGHT: continuous:false (or omit) + direct Lakeflow Connect pipeline schedules
{"continuous": false, "ingestion_definition": {/* ... */}}
```

Define custom schedules on the pipeline for recurring ingestion. Lakeflow Connect creates a Databricks Job for each schedule.

---

## Important

- **Scheduled or on-demand execution** — define one or more custom schedules directly on the pipeline, or trigger runs on demand. Lakeflow Connect auto-creates a Databricks Job per schedule. Check the connector reference for connector-specific schedule limits.
...
- **databricks-jobs** — orchestrate ingestion pipelines with downstream tasks when a larger workflow needs dependencies.

---

## Row Filtering During Ingestion

Lakeflow Connect supports GA row filtering for managed ingestion pipelines. Use filters to reduce source load, destination storage, and downstream processing by applying SQL `WHERE`-like conditions during both the initial load and incremental updates.

Supported patterns:

- **SaaS connectors** where the connector exposes filterable fields
- **Query-based connectors** where the query or connector configuration can apply predicates
- **Initial load and incremental updates** so the destination only receives rows matching the filter
...
Validate filter syntax against the connector documentation and source field names. Prefer server-side filters that align with source indexes or partition fields.

---

## Scheduling Model

Lakeflow Connect pipelines can define multiple custom schedules directly on the pipeline. Lakeflow Connect creates and manages a Databricks Job for each schedule, so do not tell users that recurring ingestion requires a separate manually-authored job wrapper.

Use direct pipeline schedules when the user asks for hourly, daily, weekly, or multiple cadence ingestion from the same connector. Reserve external Jobs orchestration for multi-step workflows that need dependencies around the ingestion pipeline.

---

## Free Tier

Lakeflow Connect includes a free tier of **100 free DBUs/day**, supporting up to **100M records daily** across managed connectors. Treat this as planning guidance and confirm current pricing/entitlement for the user's cloud, region, and account before quoting costs.
