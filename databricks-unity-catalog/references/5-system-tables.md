...
Aggregated job run records (use for high-level job run summaries).
...
Zerobus Ingest ingestion records — tracks direct-write ingestion events into Unity Catalog tables.

```sql
-- Recent Zerobus ingest activity
SELECT
    table_full_name,
    producer_id,
    ingest_time,
...
WHERE ingest_time >= current_date() - 7
...
Zerobus streaming records — tracks streaming state for Zerobus ingest streams.

```sql
-- Active Zerobus streams
SELECT
    stream_id,
    table_full_name,
    stream_state,
...
## 2025-2026 System Tables

New system tables introduced in 2025-2026. Enable the corresponding system schemas before querying.

| Table | Schema | Description |
|-------|--------|-------------|
| `system.ai.ai_gateway_requests` | `system.ai` | AI Gateway request logs (model serving gateway) |
| `system.ai.endpoint_usage` | `system.ai` | AI endpoint usage metrics |
| `system.mlflow.model_versions` | `system.mlflow` | MLflow model version registry records |
| `system.mlflow.experiments` | `system.mlflow` | MLflow experiment metadata |
| `system.serving.endpoint_usage` | `system.serving` | Model serving endpoint usage and traffic |
| `system.access.inbound_network_events` | `system.access` | Inbound network access events |
| `system.access.outbound_network_events` | `system.access` | Outbound network access events |
| `system.data_classification.label_history` | `system.data_classification` | History of data classification label changes |
| `system.quality_monitor.monitor_run_history` | `system.quality_monitor` | Data quality monitor run history |
| `system.quality_monitor.profile_metrics` | `system.quality_monitor` | Aggregated profile metrics from quality monitors |
| `system.assistant_events` | `system` | Databricks Assistant interaction events |
| `system.clean_room_events` | `system` | Clean room collaboration events |
| `system.marketplace.listing_views` | `system.marketplace` | Marketplace listing view events |
| `system.marketplace.install_events` | `system.marketplace` | Marketplace product install events |
| `system.compute.instance_events` | `system.compute` | Compute instance lifecycle events |
| `system.compute.instance_pool_events` | `system.compute` | Instance pool scaling and state events |
| `system.sharing.materialization_history` | `system.sharing` | Delta Sharing materialization history |

### Example Queries for New Tables

```sql
-- AI Gateway: recent request volume by model
SELECT
    DATE(request_time) AS request_date,
    model_name,
    COUNT(*) AS request_count,
    AVG(response_time_ms) AS avg_response_ms
FROM system.ai.ai_gateway_requests
WHERE request_time >= current_date() - 7
GROUP BY DATE(request_time), model_name
ORDER BY request_date DESC, request_count DESC;

-- Model serving endpoint usage summary
SELECT
    endpoint_name,
...
-- MLflow model versions registered recently
SELECT
    name AS model_name,
    version,
    current_stage,
...
WHERE creation_timestamp >= current_date() - 30
ORDER BY creation_timestamp DESC
LIMIT 50;

-- Data classification label changes
SELECT
    table_full_name,
    column_name,
    old_label,
    ...
    changed_by,
    change_time
FROM system.data_classification.label_history
WHERE change_time >= current_date() - 30
ORDER BY change_time DESC;

-- Quality monitor run history
SELECT
    monitor_name,
    run_status,
    run_start_time,
    run_end_time,
    rows_processed
FROM system.quality_monitor.monitor_run_history
WHERE run_start_time >= current_date() - 7
ORDER BY run_start_time DESC;

-- Marketplace install events
SELECT
    listing_id,
    listing_name,
    installed_by,
    install_time,
    install_status
FROM system.marketplace.install_events
WHERE install_time >= current_date() - 30
