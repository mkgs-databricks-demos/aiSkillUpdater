...
- **Serverless limitation**: The Zerobus SDK cannot pip-install on serverless compute. Use classic compute clusters, or use the [Zerobus REST API](https://docs.databricks.com/ingestion/zerobus-rest-api) (Beta) for notebook-based ingestion without the SDK. Use the REST API as an alternative when gRPC is unavailable in serverless environments.
...
Zerobus Ingest supports a REST API for environments where gRPC is not available.
Endpoint: POST /api/2.0/zerobus/ingest/{table_fqn}/records
...
Zerobus Ingest accepts OTLP-formatted telemetry data (traces, metrics, logs).
Configure your OTEL exporter endpoint to point to the Databricks Zerobus endpoint.
Automatically maps OTLP schema to Unity Catalog table columns.
