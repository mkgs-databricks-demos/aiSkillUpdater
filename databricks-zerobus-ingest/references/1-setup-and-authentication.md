...
Use the REST API when gRPC is unavailable (e.g., serverless environments, simple HTTP clients, webhooks):

```python
import requests

# REST API (Beta) — use when gRPC unavailable
...
response = requests.post(
    f"https://{host}/api/2.0/zerobus/ingest/{catalog}.{schema}.{table}/records",
    headers={"Authorization": f"Bearer {token}"},
    json={"records": [{"col1": "val1", "col2": 42}]}
)
...
**Notes:**
- Auth uses a Bearer token identical to other Databricks REST APIs (PAT or OAuth token)
- The `table_fqn` in the URL is the three-part Unity Catalog name: `catalog.schema.table`
- Use when the Zerobus SDK cannot be installed (serverless compute, Lambda functions, webhooks)

---

## OTLP / OpenTelemetry Integration (Beta)

Zerobus Ingest accepts OTLP-formatted telemetry data (traces, metrics, logs). Configure your OpenTelemetry exporter to point to the Databricks Zerobus endpoint:
...
# Configure OTEL exporter to send to Databricks Zerobus
exporter = OTLPSpanExporter(
    endpoint="https://<workspace-host>/api/2.0/zerobus/otlp",
    headers={"Authorization": f"Bearer {token}"},
)
...
**Notes:**
- OTLP schema is automatically mapped to Unity Catalog table columns
- Supports traces, metrics, and logs via the OTLP protocol
- Use the same Bearer token authentication as the REST API
