...
Arrow Flight provides ~40x throughput improvement over standard gRPC by writing PyArrow RecordBatches directly. Requires the `[arrow]` extra and Python 3.9+; uses a PyO3/Rust backend internally.

```python
# Arrow Flight (Beta) — 40x throughput vs standard gRPC
# pip install databricks-zerobus-ingest-sdk[arrow]
# Requires Python 3.9+, uses PyO3/Rust backend

from databricks.zerobus.ingest import ArrowFlightClient
...
# Write PyArrow RecordBatches directly
import pyarrow as pa
batch = pa.record_batch({"col1": ["a", "b"], "col2": [1, 2]})
client.write_batch(batch)
```

**Notes:**
- Install with: `pip install databricks-zerobus-ingest-sdk[arrow]`
- Best suited for bulk/batch ingestion scenarios where maximum throughput is required
- Uses binary columnar format; schema must match the target UC table

---

## Fire-and-Forget Ingestion Mode

Use `ack_mode="none"` for maximum throughput when durability confirmation is not required:

```python
# Fire-and-forget mode — no ACK, maximum throughput
client.ingest(records, ack_mode="none")
```

**Notes:**
- No ACK is returned; records may be lost if the connection drops before they are persisted
- Use only when occasional data loss is acceptable (e.g., high-frequency metrics, telemetry)
- Combine with periodic `flush()` calls to reduce loss window

---

## Offset-Based Ingestion Mode

Resume from the last committed offset after a restart or failure:

```python
# Offset-based ingestion — resume from last committed offset
client.ingest(records, start_offset=last_offset)
last_offset = client.get_committed_offset()
```

**Notes:**
- Persist `last_offset` in durable storage (e.g., a Delta table, a database) between runs
- On restart, pass `start_offset=last_offset` to avoid re-ingesting already-committed records
- Combine with at-least-once delivery semantics; downstream consumers should still deduplicate
