...
> **Recommended command (CLI v1.x):** use **`databricks auth login --host <workspace-url> --profile <profile-name>`** — this is the recommended authentication command in Databricks CLI v1.x (current v1.6.0) and performs the OAuth browser flow. `databricks configure` remains available (token/PAT-oriented, writes `~/.databrickscfg`), but prefer `databricks auth login` for OAuth.
>
> **`.databrickscfg` format note (CLI v1.0):** the CLI stores credentials/profiles in `~/.databrickscfg`. The file format has evolved across CLI v1.0 releases (e.g. how OAuth tokens/fields are stored). Let the CLI manage the file via `databricks auth login` / `databricks configure` rather than hand-editing it, and if you upgraded from a pre-v1.0 CLI, re-run `databricks auth login` to refresh profile entries into the current format.
