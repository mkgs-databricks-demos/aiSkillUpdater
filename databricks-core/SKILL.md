...
1. **CLI installed**: Run `databricks --version` to check.
   - **If the CLI is missing or outdated (< v1.0.0): STOP. Do not proceed or work around a missing CLI.** (Current CLI at time of writing is v1.6.0.)
...
> **Bundle branding:** the `databricks bundle` commands manage **Declarative Automation Bundles** (formerly *Databricks Asset Bundles*). See the `databricks-dabs` skill for full bundle guidance.

> **Authentication (CLI v1.x):** `databricks auth login --host <URL> --profile <PROFILE>` is the **recommended** way to authenticate in Databricks CLI v1.x (OAuth-based). `databricks configure` still exists for token/PAT-style setup, but prefer `databricks auth login`. See [CLI Authentication](databricks-cli-auth.md).
