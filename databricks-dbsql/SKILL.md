...
| Transactions (SQL Scripting) | `BEGIN ATOMIC...END` compound block | Preview | [references/sql-scripting.md](references/sql-scripting.md) |
...
### Databricks Runtime / DBSQL Channel Reference

| Runtime / Channel | Notes |
|-------------------|-------|
| DBR 16.3+ | SQL Scripting (compound statements, control flow) GA |
| DBR 17.0+ | Stored Procedures (Public Preview), Recursive CTEs |
| **DBR 17.3 LTS** | Current long-term-support runtime; recommended baseline for new DBSQL work |
| **Preview channel (19.x features)** | Opt-in channel that surfaces upcoming 19.x features early; use for previewing not-yet-GA capabilities. Verify GA status before relying on Preview-channel behavior in production |

> **Note on `BEGIN ATOMIC...END`:** This is a **SQL Scripting compound-statement / stored-procedure** construct (SQL/PSM), *not* a general-purpose transaction syntax. Use it to group statements inside a script or procedure body; it is not the way to open ad-hoc multi-statement transactions from arbitrary clients. See the multi-statement-transactions section of the SQL Scripting reference for the full picture, including the Python connector `autocommit = False` pattern.
...
- **Materialized view refreshes run on a serverless pipeline, not a warehouse class** — each MV automatically provisions a serverless Lakeflow pipeline that performs creation and refresh; you do not size a warehouse for the refresh itself
