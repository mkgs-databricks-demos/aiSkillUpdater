...
- **Serverless pipelines**: Each MV automatically creates and refreshes via a **serverless (Lakeflow) pipeline** — the refresh is backed by a serverless pipeline, **not** by a SQL warehouse class. You do not size or select a warehouse for the refresh compute; Databricks manages it as a serverless pipeline
