...
> **Scope:** `BEGIN ATOMIC...END` is a **SQL Scripting compound-statement / stored-procedure** construct (SQL/PSM). It is *not* general-purpose transaction syntax for arbitrary client sessions. Use it inside a SQL script or a `CREATE PROCEDURE` body to make the enclosed compound statement atomic. For client-driven multi-statement transactions, use the Python connector `autocommit = False` pattern below.

Use `BEGIN ATOMIC...END` to execute the statements of a compound block as a single atomic unit:
...
| **DBR 17.3 LTS (baseline runtime)** | 17.3 | GA — recommended LTS baseline for DBSQL work |
| **Preview channel (19.x features)** | Preview channel | Opt-in early access to upcoming 19.x features; verify GA before production use |
