...
> **`aitools` is plugin-first in CLI v1.6.0**: the `aitools` functionality is delivered as a CLI plugin in Databricks CLI v1.6.0. The invocation may be `databricks aitools ...` (plugin) rather than `databricks experimental aitools ...`. **Verify the exact subcommand path with `databricks aitools --help`** (and `databricks experimental aitools --help`) on your installed CLI, and use whichever your version exposes.
...
> **Verify the tag-assignment subcommand**: the `workspace-entity-tag-assignments` command surface may differ between CLI versions. Confirm it exists and check its exact syntax against `databricks workspace --help` (and `databricks workspace-entity-tag-assignments --help`) in CLI v1.6.0 before relying on it in automation.
