...
**Requirements**: Unity Catalog, DBR 16.4 LTS+ (Managed Iceberg v2), DBR 18.0+ (Managed Iceberg v3 GA)
...
## Predictive Optimization (Required for Automatic Maintenance)

**Predictive Optimization MUST be enabled** on managed Iceberg tables for automatic maintenance. Without it, tables accumulate small files over time, degrading query performance and increasing storage costs.

Predictive Optimization is **not auto-enabled** and must be turned on explicitly. Once enabled, it automatically runs:
...
Enable at the table level with TBLPROPERTIES:

```sql
-- Enable Predictive Optimization on a managed Iceberg table
ALTER TABLE my_catalog.my_schema.events
SET TBLPROPERTIES ('delta.enablePredictiveOptimization' = 'auto');
```

Or enable at the catalog or schema level (applies to all tables within):

```sql
-- Enable for all tables in a schema
ALTER SCHEMA my_catalog.my_schema
SET DBPROPERTIES ('delta.enablePredictiveOptimization' = 'auto');
...
Manual operations are still available if needed:
...
**Requires**: DBR 18.0+  
**Geospatial types in Iceberg v3 require DBR 18.2+**

Iceberg v3 is now generally available (GA) and introduces new capabilities on top of v2:

| Feature | Description |
|---------|-------------|
| **Deletion Vectors** | Row-level deletes without rewriting data files — faster UPDATE/DELETE/MERGE. **Enabled by default on new v3 tables.** |
...
| **Geospatial Types** | Requires DBR 18.2+ |
...
### Deletion Vectors on Iceberg v3

Deletion vectors (DVs) are **enabled by default** on new Iceberg v3 tables. The controlling property is `'iceberg.enableDeletionVectors'`.

If you need to use `CLUSTER BY` on a v3 table with DVs disabled (e.g., for compatibility with external engines that don't support DVs):

```sql
-- Disable deletion vectors on a v3 table (required if CLUSTER BY is needed with DV-incompatible readers)
CREATE TABLE my_catalog.my_schema.events_v3_no_dv (
  event_id BIGINT,
  event_date DATE
)
USING ICEBERG
...
Or disable on an existing table:

```sql
ALTER TABLE my_catalog.my_schema.events_v3
SET TBLPROPERTIES ('iceberg.enableDeletionVectors' = 'false');
```

### Important Notes

- **Cannot downgrade**: Once a table is upgraded to v3, it cannot be downgraded back to v2
- **External engine compatibility**: External engines must use Iceberg library 1.9.2+ to read v3 tables
- **Deletion vectors**: Enabled by default on v3 tables via `'iceberg.enableDeletionVectors'`. External readers must support deletion vectors (or disable DVs on the table)
- **GA status**: Iceberg v3 is GA as of DBR 18.0 — suitable for production workloads
