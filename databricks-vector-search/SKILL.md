...
description: "Databricks AI Search endpoints and indexes for RAG and semantic search; covers index types, search modes, end-to-end RAG patterns"
metadata:
  version: "0.2.0"
...
| **Storage-Optimized** | 300-500ms | 1B+ vectors (768 dim) | 7x lower | Large-scale, cost-sensitive; also required for Full-Text Search |
...
| **Full-Text Search (Beta)** | None (BM25 keyword) | Auto from Delta | Keyword search without embeddings |
...
```python
# For large-scale, cost-effective deployments (also required for Full-Text Search)
endpoint = w.vector_search_endpoints.create_endpoint(
...
Full-Text Search indexes use BM25 keyword scoring and require no embeddings. They require a Storage-Optimized endpoint (`endpoint_type='STORAGE_OPTIMIZED'`).

```python
# Requires a Storage-Optimized endpoint
endpoint = w.vector_search_endpoints.create_endpoint(
    name="my-fts-endpoint",
    endpoint_type="STORAGE_OPTIMIZED"
)

# Create Full-Text Search index (no embedding model needed)
index = w.vector_search_indexes.create_index(
...
results = index_client.similarity_search(
    query_text="machine learning pipeline",
    columns=["id", "content"],
    num_results=10,
...
Storage-Optimized endpoints use SQL-like filter syntax via the `databricks-ai-search` package's `filters` parameter (accepts a string):

```python
from databricks.ai_search.client import AISearchClient

client = AISearchClient()
index = client.get_index(endpoint_name="my-storage-endpoint", index_name="catalog.schema.my_index")
...
|-------|------|-------------|
| Index Types | [references/index-types.md](references/index-types.md) | Detailed comparison of Delta Sync (managed/self-managed) vs Direct Access vs Full-Text Search |
...
| Search Modes | [references/search-modes.md](references/search-modes.md) | When to use semantic (ANN) vs hybrid vs full-text search, decision guide |
...
| **filters_json not working** | Storage-Optimized uses SQL-like string filters via `databricks-ai-search` package's `filters` parameter |
...
| **Full-Text Search endpoint error** | Full-Text Search indexes require a Storage-Optimized endpoint (`endpoint_type='STORAGE_OPTIMIZED'`) |
...
|-------|------------|----------------|----------|
| `databricks-qwen3-embedding-0-6b` | 1024 | 32768 tokens | **Production default** — high quality, efficient |
| `databricks-gte-large-en` | 1024 | 8192 tokens | English text, legacy (still available) |
| `databricks-bge-large-en` | 1024 | 512 tokens | English text, legacy (still available) |

```python
# Use with managed embeddings — current production default
embedding_source_columns=[
    {
        "name": "content",
        "embedding_model_endpoint_name": "databricks-qwen3-embedding-0-6b"
...
- **Full-Text Search (Beta)** — keyword-only BM25 search, no embeddings, requires Storage-Optimized endpoint
- **`columns_to_sync` matters** — only synced columns are available in query results; include all columns you need
- **Filter syntax differs by endpoint** — Standard uses dict-format filters, Storage-Optimized uses SQL-like string filters. Use the `databricks-ai-search` package's `filters` parameter which accepts both formats
...
- **databricks-pipelines** - Build Delta tables used as AI Search sources
