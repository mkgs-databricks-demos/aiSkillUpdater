# Databricks AI Search Troubleshooting & Operations
...
**Recommendation:** Start with Storage-Optimized unless you need sub-100ms latency. It handles most RAG workloads well and is required for Full-Text Search indexes.
...
delta_sync_index_spec={
    ...
    "embedding_source_columns": [
        {"name": "content", "embedding_model_endpoint_name": "databricks-qwen3-embedding-0-6b"}
    ],
...
| **filters_json has no effect** | Using wrong filter syntax for endpoint type | Standard endpoints use dict-format filters (`filters_json` in SDK, `filters` as dict in `databricks-ai-search`). Storage-Optimized endpoints use SQL-like string filters (`filters` as str in `databricks-ai-search`). |
...
| **Full-Text Search endpoint error** | Wrong endpoint type | Full-Text Search indexes require a Storage-Optimized endpoint. Create endpoint with `endpoint_type="STORAGE_OPTIMIZED"`. |
| **`ImportError: databricks.vector_search`** | Old package name | Install updated package: `pip install databricks-ai-search`. Import from `databricks.ai_search`. |
