# Databricks AI Search Modes

Databricks AI Search supports three search modes: **ANN** (semantic, default), **HYBRID** (semantic + keyword), and **FULL_TEXT** (keyword only, beta). ANN and HYBRID work with Delta Sync and Direct Access indexes. FULL_TEXT requires a Full-Text Search index on a Storage-Optimized endpoint.
...
Full-Text Search uses BM25 keyword scoring without any vector embeddings. It requires a Full-Text Search index on a Storage-Optimized endpoint.
...
- Scenarios where you want deterministic keyword results
...
results = index.similarity_search(
    query_text="executor memory OOM error",
    columns=["id", "content"],
    num_results=10,
...
| **FULL_TEXT** (beta) | Pure keyword search without vector embeddings | No semantic understanding; max 200 results; requires Storage-Optimized endpoint + Full-Text index | You need keyword matching only, without vector similarity |

**Start with ANN.** Switch to HYBRID if you notice relevant documents being missed because they don't share vocabulary with the query. Use FULL_TEXT only when embeddings are not needed or not available.

## Combining Search Modes with Filters

Both ANN and HYBRID search modes support filters. The filter syntax depends on your endpoint type:

- **Standard endpoints** → `filters` as dict (or `filters_json` as JSON string via `databricks-sdk`)
- **Storage-Optimized endpoints** → `filters` as SQL-like string (via `databricks-ai-search` package)
...
- For **FULL_TEXT** queries: provide only `query_text`; no embeddings are used.

## Parameter Reference

| Parameter | Type | Package | Description |
|-----------|------|---------|-------------|
| `query_text` | `str` | Both | Text query — requires embedding model on the index (or Full-Text index for FULL_TEXT mode) |
| `query_vector` | `list[float]` | Both | Pre-computed embedding vector |
| `query_type` | `str` | Both | `"ANN"` (default), `"HYBRID"`, or `"FULL_TEXT"` (beta) |
| `columns` | `list[str]` | Both | Column names to return in results |
| `num_results` | `int` | Both | Number of results (default: 10 in `databricks-sdk`, 5 in `databricks-ai-search`) |
| `filters_json` | `str` | `databricks-sdk` | JSON dict filter string (Standard endpoints) |
| `filters` | `str` or `dict` | `databricks-ai-search` | Dict for Standard, SQL-like string for Storage-Optimized |
