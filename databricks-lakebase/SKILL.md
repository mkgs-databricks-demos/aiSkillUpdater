...
- **Branching** -- first-class copy-on-write branches with TTL, point-in-time recovery, reset, and `databricks lakebase branches` / `databricks postgres` CLI workflows
...
- **Branch**: Isolated database environment sharing storage with parent (copy-on-write). Branches are first-class resources for dev/test, recovery, and isolated changes. States include `READY` and `ARCHIVED`.
...
Use branches whenever users need isolated development, testing, recovery, or short-lived data copies without duplicating a full database. Lakebase branches are copy-on-write: child branches initially share storage with their parent and only store changed data after divergence.

Key branch capabilities:

- **Copy-on-write clones** for development, QA, migrations, and production hotfix validation
- **Point-in-time recovery** to create a branch from an earlier timestamp before a destructive change
- **TTL policies** for short-lived branches so ephemeral environments expire automatically
- **Branch reset** to bring a branch back to a parent or recovery point when the CLI/API exposes reset for the workspace
- **CLI management** through branch commands; discover exact syntax with `databricks lakebase branches -h` when available, and fall back to `databricks postgres list-branches`, `create-branch`, and related `postgres` commands in current workspaces

Recommended workflow:

```bash
# Discover the branch command surface first; command groups can vary by CLI/workspace rollout.
databricks lakebase branches -h || databricks postgres -h

# Existing postgres command group examples.
databricks postgres list-branches <PROJECT_ID> --profile <PROFILE>
databricks postgres create-branch <PROJECT_ID> <BRANCH_ID>   --json '{"spec": {"parent_branch": "production"}}'   --profile <PROFILE>
```

When creating app environments, ask whether the user wants to reuse `production`, create a long-lived environment branch such as `staging`, or create a TTL-bound feature branch.
