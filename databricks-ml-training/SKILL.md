...
> **Python 3.10 minimum**: Databricks SDK v0.103.0+ (released Apr 2026) dropped support for Python 3.8 and 3.9. Ensure your environment uses Python 3.10+.
...
## MLflow 3 LoggedModel Paradigm

MLflow 3 introduces a new `LoggedModel` abstraction that decouples model logging from runs:

```python
import mlflow

# Models can be logged without starting a run
logged_model = mlflow.initialize_logged_model(
    name="my_model",
    params={"n_estimators": 100, "max_depth": 5}
)

# Set as the active model for subsequent logging
mlflow.set_active_model(model_id=logged_model.model_id)

# Log artifacts, metrics, etc.
...
| API | Description |
|-----|-------------|
| `mlflow.initialize_logged_model(name=..., params=...)` | Create a LoggedModel without a run |
| `mlflow.set_active_model(model_id=...)` | Set the active LoggedModel for subsequent logging |
| `mlflow.finalize_logged_model(model_id, status=...)` | Mark a LoggedModel as complete |
| `mlflow.search_logged_models(...)` | Search across LoggedModels |
| `mlflow.create_external_model(...)` | Register a model hosted outside MLflow |

### Model URIs

- **New canonical URI**: `models:/<model_id>` (model ID, not name)
- **Old `runs:/` URIs**: Still work for backward compatibility
- **Named model with alias**: `models:/{catalog}.{schema}.{model}@prod` (still works)

### Deprecation: `artifact_path` → `name=`

```python
# OLD (still works but deprecated in MLflow 3)
mlflow.xgboost.log_model(model, artifact_path="model", registered_model_name=FULL_NAME)

# NEW (MLflow 3 style)
mlflow.xgboost.log_model(model, name="model", registered_model_name=FULL_NAME)
```

### Backward Compatibility

`mlflow.xgboost.log_model(registered_model_name=...)` and other framework-specific `log_model` calls still work in MLflow 3.x — no forced migration required.
...
## mlflow.evaluate() Deprecation

> **DEPRECATED in MLflow 3.0.0**: `mlflow.evaluate()` is deprecated. Use the appropriate replacement:
> - Traditional ML evaluation: `mlflow.models.evaluate()`
> - GenAI / LLM evaluation: `mlflow.genai.evaluate()`
>
> `mlflow.evaluate()` will be **removed in MLflow 3.7.0**.

```python
# DEPRECATED — do not use
mlflow.evaluate(model_uri, data, targets=..., model_type=...)

# REPLACEMENT for traditional ML
mlflow.models.evaluate(model_uri, data, targets=..., model_type=...)

# REPLACEMENT for GenAI/LLM
mlflow.genai.evaluate(data=eval_dataset, predict_fn=..., scorers=[...])
```
...
RUN_ID=$(databricks jobs submit --no-wait --json '{
  ...
  "environments": [{
    ...
    "spec": {
      "client": "4",
      "dependencies": ["mlflow==3.14.0", "xgboost==2.1.3", "optuna==4.1.0", "scikit-learn==1.5.2"]
    }
  }]
}' | jq -r .run_id)
```

> **Note on serverless environment versions**: `client: "4"` is the current default for serverless notebook tasks. Environment v5 is now available as the latest — specify `"client": "5"` to use it. v5 includes updated pre-installed packages and Python 3.11.
...
| `mlflow.evaluate()` raises deprecation warning | Use `mlflow.models.evaluate()` for traditional ML or `mlflow.genai.evaluate()` for GenAI |
| Python 3.8/3.9 import errors with SDK v0.103.0+ | SDK dropped Python 3.8/3.9 in Apr 2026. Upgrade to Python 3.10+. |
| Used `artifact_path=` in MLflow 3 | Use `name=` instead; `artifact_path` is deprecated in MLflow 3 |
...
- **[databricks-ai-search](../databricks-vector-search/SKILL.md)** — vector indexes used as retrieval tools in agents.
