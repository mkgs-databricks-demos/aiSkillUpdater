...
> **AppKit web docs:** the authoritative AppKit v0 documentation is at
> **https://developers.databricks.com/docs/appkit/v0/** (the older `databricks.github.io/appkit`
> location is deprecated). Prefer the `npx @databricks/appkit docs` CLI for the exact version
> installed in your project.
>
> **Verify names against current AppKit v0 docs:** the `useGenieChat` hook and the `genie()` plugin
> scope/space names referenced in this skill are versioned and may change. Before relying on them,
> confirm the current hook signature (`useGenieChat`) and the `genie()` plugin scope names against
> `npx @databricks/appkit docs ./docs/plugins/genie.md` (and the AppKit v0 web docs above).
