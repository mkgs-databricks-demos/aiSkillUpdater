...
Match the serverless environment version to the DBR version the workload was running on. Serverless environment versions form a ladder — **v5 is the current latest**:

| Classic DBR Version | Serverless `spec.client` | Python | Notes |
|---------------------|--------------------------|--------|-------|
| 13.x, 14.x | `"1"` | 3.10 | legacy |
| 15.x | `"2"` | 3.11 | |
| 16.x+ | `"3"` | 3.12 | |
| 16.x+ | `"4"` | 3.12 | JAR job tasks require environment version 4+ |
| latest | `"5"` | 3.12 | **current** — prefer for new serverless workloads |

Ladder: **v1 (legacy) → v2 → v3 → v4 → v5 (current)**. Pick the highest version compatible with your dependencies; verify the exact Python/library set for each version against the current serverless environment-version docs.

### Documented serverless limits

| Limit | Value | Notes |
|-------|-------|-------|
| Maximum runtime per serverless run | **7 days** | A serverless workload cannot run longer than 7 days; long-running jobs must checkpoint/split |
| `spark.createDataFrame` local data cap | **128 MB** | Creating a DataFrame from local (driver-side) Python data is capped at 128 MB of rows; for larger data read from a table/files instead |
