...
| Pattern | Grep/Regex | Severity | Fix | Docs |
|---------|-----------|----------|-----|------|
| `df.cache()` | `\.cache\(\)` | Warning | Remove caching calls. Materialize expensive results to Delta tables. (Native `cache()` support on serverless is **unverified** — do not rely on it.) | [Limitations](https://docs.databricks.com/en/compute/serverless/limitations) |
