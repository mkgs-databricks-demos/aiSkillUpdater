...
| `df.cache()` / `df.persist()` | Warning | Remove caching calls. For expensive intermediate results, materialize to a Delta table. (Native `cache()`/`persist()` support on serverless is **unverified** — do not rely on it; verify against current serverless limitations docs.) |
...
| Retired Foundation Model endpoint references, e.g., `databricks-meta-llama-3-1-405b-instruct` and similar (D1) | Blocker | Detect by **content scan across every migrated file** (not by filename pattern). Common refs in `ai_query(endpoint => '...')`, `ChatDatabricks(endpoint=...)`, model-serving config, and Genie/AI-Functions SQL. Replace with a currently-available Foundation Model endpoint. `databricks-meta-llama-3-3-70b-instruct` is a common choice but is **no longer 'the current default'** — Llama 4, GPT-5.x, and Qwen3 endpoints are also available. **Verify the chosen replacement endpoint exists in the target workspace before final deploy** (endpoint availability varies by workspace/region). |
...
# AFTER (serverless — remove .cache(); native cache()/persist() support is UNVERIFIED, do not rely on it):
df = spark.read.parquet(path)
result1 = df.filter("status = 'active'")
result2 = df.groupBy("region").agg(F.sum("revenue"))
...
Remove `job_clusters`/`new_cluster`, add `environments` with serverless spec, replace `job_cluster_key` with `environment_key`, remove `init_scripts`. See [Configuration Guide](references/configuration-guide.md) for full before/after JSON and the environment version mapping (ladder **v1-legacy → v2 → v3 → v4 → v5-current**).

**Documented serverless limits**: maximum runtime per serverless run is **7 days**; `spark.createDataFrame` from local (driver-side) Python data is capped at **128 MB** of rows. See the Configuration Guide's limits table.
...
| 404 on `ai_query(endpoint => 'databricks-meta-llama-3-1-405b-instruct')` | D1: retired Foundation Model endpoint. Replace with a currently-available endpoint (e.g. `databricks-meta-llama-3-3-70b-instruct`, Llama 4, GPT-5.x, or Qwen3 — no single 'current default') via content scan across all migrated files; verify it exists in the target workspace first. |
