...
- **Python 3.10 minimum**: the Databricks SDK for Python requires Python **3.10+** (SDK v0.103.0+ dropped support for Python 3.8 and 3.9)
...
# DEPRECATED (Feb 2026): w.serving_endpoints.get_open_ai_client() is deprecated.
# Prefer the dedicated databricks-openai package instead:
#     pip install databricks-openai
#     from databricks.openai import DatabricksOpenAI
#     openai_client = DatabricksOpenAI().client   # OpenAI-compatible client
openai_client = w.serving_endpoints.get_open_ai_client()  # legacy; deprecated Feb 2026
```

> **Deprecation (Feb 2026):** `workspace_client.serving_endpoints.get_open_ai_client()` is
> deprecated. Use the `databricks-openai` package: `pip install databricks-openai`, then
> `from databricks.openai import DatabricksOpenAI`.
