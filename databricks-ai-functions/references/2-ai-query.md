...
|---|---|---|---|
| `endpoint` | STRING literal | — | Foundation Model name or custom endpoint name. Never guess — use exact names from the current model roster below. |
...
These are the current (July 2026) Foundation Model API endpoints. Endpoint names change as models are updated — always verify against the [model serving docs](https://docs.databricks.com/machine-learning/foundation-models/supported-models.html) before hardcoding a name in production pipelines, or better yet, load the name from `config.yml`.

| Use case | Endpoint name |
|---|---|
| General reasoning / extraction | `databricks-claude-sonnet-4-5` |
...
> **Always load model names from `config.yml`, never hardcode them in pipeline code.** Foundation model endpoint names are versioned and rotated — a hardcoded name breaks silently when a model is retired.
...
Preferred over `returnType` for chat models:
...
def extracted():
    return (
        ...
        .withColumn("ai_response", expr("""
            ai_query('databricks-claude-sonnet-4-5', prompt,
                     responseFormat => '{"type":"json_object"}',
                     failOnError     => false)
        """))
    )
