...
**2. Submit the run.** Use `--no-wait` to get `{"run_id": N}` back immediately; drop it to block until terminated. **Use serverless environment version `"5"` in new examples.** Environment versions are a ladder (`"1"` through `"5"`); `"5"` is the current latest and supports dependency installation. Older examples using `"4"` still work where supported, but do not author new snippets with `"4"`.
...
Serverless job environments use a `client` version ladder from `"1"` through `"5"`. Use `"5"` for new examples because it is the current latest environment version. Do not reuse older environment-version snippets unless you are preserving compatibility with an existing job.
...
Timeouts are configured at the task or job level with `timeout_seconds`. Do not describe serverless jobs as having a fixed platform timeout cap.
