...
## ConversationSimulator — Generate Multi-Turn Eval Datasets

Use `mlflow.genai.ConversationSimulator` (MLflow 3.14.0+) to automatically generate realistic multi-turn conversation datasets for evaluation without manually authoring every exchange.
...
# dataset is an mlflow EvaluationDataset ready for mlflow.genai.evaluate()

results = mlflow.genai.evaluate(
    data=dataset,
    predict_fn=my_app,
    scorers=[Safety(), Guidelines(name="helpful", guidelines="Must be helpful")]
)
```

**Key parameters:**
- `system_prompt` — The system prompt that will be used by the agent being simulated against.
- `num_turns` — Number of back-and-forth turns per conversation (default: 5).
- `num_conversations` — Number of distinct conversations to generate (default: 10).

**Notes:**
- The generated dataset includes realistic user messages that probe different aspects of the agent's behavior.
- Useful for bootstrapping eval datasets when you don't have production traces yet.
- Requires MLflow 3.14.0+.
