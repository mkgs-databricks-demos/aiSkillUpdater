...
**Version**: MLflow 3.14.0+ (mlflow[databricks]>=3.14.0)
...
- [LoggedModel Lifecycle APIs](#loggedmodel-lifecycle-apis)
...
from mlflow.genai.judges import (
    ...
    is_context_relevant,
    is_grounded,
    make_judge,  # DEPRECATED in MLflow 3.14.0 — use @scorer with Guidelines instead
)
```
...
### make_judge() - Custom LLM Judge (DEPRECATED)

> **Deprecated in MLflow 3.14.0**: `make_judge()` is deprecated. Use `@scorer` with `Guidelines` or custom scoring logic instead.
...
### mlflow.genai.create_monitor() (MLflow 3.14.0+)

The new high-level API for production monitoring. Creates and starts a monitor in a single call.
...
### Register and Start Scorer (lower-level alternative)
```python
from mlflow.genai.scorers import Safety, Guidelines, ScorerSamplingConfig
...
```python
# Set the active LoggedModel for the current context
# Subsequent logging calls (metrics, params, artifacts) attach to this model
mlflow.set_active_model(model_id="<MODEL_ID>")
```

### mlflow.initialize_logged_model()
```python
# Create a new LoggedModel record
model = mlflow.initialize_logged_model(
    name="my-agent",
    params={"max_tokens": 1024, "temperature": 0.7}
)
print(model.model_id)  # Use this ID with set_active_model()
```

### mlflow.finalize_logged_model()
```python
# Mark a LoggedModel as finalized (ready for promotion or evaluation)
mlflow.finalize_logged_model(model_id="<MODEL_ID>")
```

### mlflow.search_logged_models()
```python
# Search LoggedModel records by filter expression
models = mlflow.search_logged_models(
    filter_string="name = 'my-agent'"
)

# Filter by params
models = mlflow.search_logged_models(
    filter_string="params.max_tokens > 512"
)

# Filter by status
models = mlflow.search_logged_models(
    filter_string="status = 'FINALIZED'"
)
```

### mlflow.create_external_model()
```python
# Register an external model (not trained in MLflow) as a LoggedModel
# Points to an external artifact source such as an S3 path or HuggingFace repo
external_model = mlflow.create_external_model(
    name="my-external-llm",
    source="s3://my-bucket/models/my-llm-v1"
)
print(external_model.model_id)
```
