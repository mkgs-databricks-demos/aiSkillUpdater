...
- [Using @mlflow.test Without Required Dependencies](#-using-mlflowtest-without-required-dependencies)
- [Using make_judge() in MLflow 3.14.0+](#-using-make_judge-in-mlflow-3140)
- [Wrong Production Monitoring — Legacy Setup vs create_monitor()](#-wrong-production-monitoring--legacy-setup-vs-create_monitor)
...
## ❌ Using @mlflow.test Without Required Dependencies

### WRONG: Using @mlflow.test without the pytest plugin installed
```python
# ❌ WRONG - Will fail if mlflow[pytest] is not installed
import mlflow

@mlflow.test
def test_agent_quality():
    results = mlflow.genai.evaluate(data=eval_data, predict_fn=my_app, scorers=[Safety()])
    assert results.metrics["safety/mean"] == 1.0
```

### ✅ CORRECT: Install the pytest plugin and use MLflow 3.14.0+
```bash
# ✅ CORRECT - Install required extras
pip install "mlflow[databricks,pytest]>=3.14.0"
```

```python
# ✅ CORRECT - @mlflow.test integrates evaluations into pytest
import mlflow

@mlflow.test
def test_agent_quality():
    """This test's results are auto-logged as an MLflow run."""
    results = mlflow.genai.evaluate(data=eval_data, predict_fn=my_app, scorers=[Safety()])
    assert results.metrics["safety/mean"] == 1.0
```

**Why?** `@mlflow.test` requires MLflow 3.14.0+ and the `pytest` extras (`pip install mlflow[pytest]`). Without these, the decorator will raise an `ImportError` or silently do nothing depending on the version.

---

## ❌ Using make_judge() in MLflow 3.14.0+

### WRONG: Using make_judge() in new code
```python
# ❌ DEPRECATED - make_judge() is removed in MLflow 3.14.0
from mlflow.genai.judges import make_judge

my_judge = make_judge(
    name="quality_check",
    instructions="Evaluate if the response is high quality. Respond yes or no.",
)
```

### ✅ CORRECT: Use @scorer with Guidelines or custom logic
```python
# ✅ CORRECT - Use @scorer decorator with Guidelines for LLM-based evaluation
from mlflow.genai.scorers import scorer, Guidelines
from mlflow.entities import Feedback

# Option A: Use Guidelines scorer for natural language criteria
quality_scorer = Guidelines(
    name="quality_check",
    guidelines="The response must be high quality, accurate, and helpful.",
)

# Option B: Use @scorer for custom logic
@scorer
def quality_check(inputs, outputs):
    """Custom quality check using inline LLM call."""
    from mlflow.genai.judges import meets_guidelines
    return meets_guidelines(
        name="quality_check",
        guidelines="The response must be high quality, accurate, and helpful.",
        context={"request": str(inputs), "response": str(outputs)},
    )
```

**Why?** `make_judge()` is deprecated as of MLflow 3.14.0 and removed in future versions. The `@scorer` decorator with `Guidelines` is the idiomatic replacement. For existing code using `make_judge()` for MemAlign workflows, the judge objects returned by `make_judge()` still work for `align()` in the 3.14.x series but will be removed in a future release.

---

## ❌ WRONG Production Monitoring — Legacy Setup vs create_monitor()

### WRONG: Forgetting that mlflow.genai.create_monitor() replaces the two-step setup
```python
# ❌ OUTDATED - Two-step register+start pattern is the legacy approach
safety = Safety().register(name="safety_check")
safety = safety.start(sampling_config=ScorerSamplingConfig(sample_rate=0.5))
```

### ✅ CORRECT: Use create_monitor() for new production monitoring setups
```python
# ✅ CORRECT - Single call, monitor is active immediately
import mlflow
from mlflow.genai.scorers import Safety, ScorerSamplingConfig

monitor = mlflow.genai.create_monitor(
    name="production_safety_monitor",
    scorers=[Safety()],
    sampling_config=ScorerSamplingConfig(sample_rate=0.5),
)
...
**Notes:**
- `mlflow.genai.create_monitor()` replaces the legacy monitor setup in MLflow 3.14.0+.
- The legacy `register()` + `start()` pattern still works but is no longer recommended for new code.
- `create_monitor()` simplifies the API: one call creates and immediately activates the monitor.
...
- [ ] Production monitoring scorers are both registered AND started (or use `create_monitor()`)
...
- [ ] `@mlflow.test` requires MLflow 3.14.0+ and `pip install mlflow[pytest]`
- [ ] `make_judge()` is deprecated in MLflow 3.14.0 — migrate to `@scorer` decorator
- [ ] For new production monitoring, use `mlflow.genai.create_monitor()` instead of the register+start pattern
