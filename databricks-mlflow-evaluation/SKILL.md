...
### Workflow 9 — ConversationSimulator

Use `mlflow.genai.ConversationSimulator` to automatically generate multi-turn conversation datasets for evaluation. Pass a system prompt and the simulator creates realistic user-agent exchanges.

| Step | Action | Reference Files |
|------|--------|-----------------|
| 1 | Configure simulator with system prompt and turn count | `patterns-evaluation.md` (ConversationSimulator section) |
| 2 | Generate multi-turn conversation dataset | `patterns-evaluation.md` (ConversationSimulator section) |
| 3 | Use generated dataset in mlflow.genai.evaluate() | `patterns-evaluation.md` (Pattern 1) |
...
- **@mlflow.test decorator:** `@mlflow.test` decorator integrates MLflow evaluations into pytest — mark test functions to auto-log results as MLflow runs. Requires MLflow 3.14.0+ and `pip install mlflow[pytest]`.
- **make_judge() deprecated:** `make_judge()` is deprecated as of MLflow 3.14.0 — use `@scorer` with `Guidelines` or custom scoring logic instead.

See `GOTCHAS.md` for complete list.

## LoggedModel Lifecycle APIs

MLflow 3.14.0 introduces a full set of LoggedModel lifecycle APIs for managing model records associated with experiments and runs:

- `mlflow.set_active_model(model_id)` — Set the active LoggedModel for the current context; subsequent logging calls attach to this model.
- `mlflow.initialize_logged_model(name, params)` — Create a new LoggedModel record with an optional params dict; returns the model object with a `model_id`.
- `mlflow.finalize_logged_model(model_id)` — Mark a LoggedModel as finalized (ready for promotion or evaluation).
- `mlflow.search_logged_models(filter_string)` — Search LoggedModel records by filter expression (e.g., `"name = 'my-agent'"` or `"params.max_tokens > 512"`).
- `mlflow.create_external_model(name, source)` — Register an external model (not trained in MLflow) as a LoggedModel, pointing to an external artifact source.

## Production Monitoring with mlflow.genai.create_monitor()

MLflow 3.14.0 introduces `mlflow.genai.create_monitor()` as the new high-level API for setting up production monitoring. It replaces the lower-level register/start scorer pattern for most use cases:
...
monitor = mlflow.genai.create_monitor(
    name="production_quality_monitor",
    scorers=[
        Safety(),
        Guidelines(name="helpful", guidelines="Responses must be helpful and professional"),
    ],
    sampling_config=ScorerSamplingConfig(sample_rate=0.5),
    experiment_id="<EXPERIMENT_ID>",  # Optional
)
# Monitor is active immediately — no separate .start() call needed
```

See `CRITICAL-interfaces.md` (Production Monitoring section) and `GOTCHAS.md` for details.
