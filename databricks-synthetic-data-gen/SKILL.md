...
## LLM-Based Synthetic Generation (DBR 18.2+ Serverless)

Use `ai_gen()` for AI-powered synthetic data that needs realistic, coherent text fields (product descriptions, reviews, support tickets, etc.).
Use `ai_query()` to generate complete structured records via a foundation model.

### ai_gen() for individual fields
```sql
SELECT 
  uuid() AS id,
  ai_gen('Generate a realistic product name for a kitchen appliance') AS product_name,
  ai_gen('Write a 2-sentence product description for a kitchen appliance') AS description,
  CAST(rand() * 500 AS DECIMAL(10,2)) AS price
FROM range(1000);
```

### ai_query() for full structured records
```python
...
df = spark.range(500).select(
    expr("""
      ai_query(
        ...
        concat('Generate a JSON object for a synthetic customer record with fields: ',
               'name, email, city, signup_date, tier (free/pro/enterprise). ',
...
## When to Use Faker vs ai_gen()

| Use Case | Tool | Reason |
|----------|------|--------|
| Structured data (dates, IDs, numbers) | Faker | Fast, deterministic, no LLM cost |
| Simple name/address/email fields | Faker | Well-tested patterns, locale support |
| Realistic product descriptions | ai_gen() | LLM creates coherent, varied text |
| Support ticket content | ai_gen() | LLM mimics natural language |
| Complex correlated records | ai_query() | LLM maintains inter-field coherence |
| Large volumes (>1M rows) | Faker | LLM cost and latency scale linearly |
| Evaluation/RAG test datasets | ai_gen()/ai_query() | Need natural language variety |
| Schema compliance critical | Faker | Deterministic type control |
