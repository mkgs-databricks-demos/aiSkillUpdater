...
## LLM-Based Synthetic Generation Patterns (DBR 18.2+ Serverless)

```python
# LLM-Based Synthetic Generation patterns (DBR 18.2+ Serverless)
# Use ai_gen() for realistic text fields

# Pattern 1: Mixed Faker + ai_gen() dataset
df = spark.range(1000).select(
    F.expr("uuid()").alias("id"),
    F.expr("ai_gen('Generate a realistic B2B company name')").alias("company_name"),
    F.expr("ai_gen('Write a one-line company tagline')").alias("tagline"),
    # Use Faker UDFs for structured fields
    generate_email_udf().alias("email"),
    F.expr("cast(rand() * 10000 as int)").alias("employee_count"),
)

# Pattern 2: ai_query() for fully coherent records
# Good for RAG test data, chatbot evaluation datasets
base_df = spark.range(100).select(
    F.expr("""
      ai_query(
        'databricks-meta-llama-3-3-70b-instruct',
        'Generate a synthetic customer support ticket as JSON: {ticket_id, subject, body, priority, category}'
      )
    """).alias("ticket_json")
).select(F.from_json("ticket_json", schema).alias("ticket")).select("ticket.*")
```
