...
| Mode | Solution |
|------|----------|
| **DB Connect with Serverless** | Install libs locally (`uv pip install faker==37.0.0`), use `DatabricksSession.builder.serverless(True)` |
...
# Install dependencies locally first: uv pip install faker==37.0.0 pandas numpy holidays
...
```python
{
  "environments": [{
    ...
    "spec": {
      "client": "4",  # Required!
      "dependencies": ["faker==37.0.0", "numpy", "pandas"]
    }
  }]
}
```
...
## ai_gen() / ai_query() Troubleshooting

**ai_gen() not found**: Requires DBR 18.2+ Serverless. Classic clusters not supported.

**Rate limiting**: ai_gen() calls the Foundation Model API. For large datasets (>100K rows), consider:
- Generating in batches with repartition()
- Using a dedicated provisioned throughput endpoint via ai_query()
- Falling back to Faker for fields where LLM variety isn't needed

**Slow generation**: ai_gen() is ~10-100x slower than Faker. Set realistic expectations:
- Faker: millions of rows in seconds
- ai_gen(): hundreds to thousands of rows per minute

**JSON parsing errors from ai_query()**: LLMs occasionally produce malformed JSON.
Use `try_parse_json()` instead of `from_json()` and filter nulls:
```sql
SELECT *
FROM (
  SELECT try_parse_json(ai_query(...)) AS parsed
  FROM range(100)
)
WHERE parsed IS NOT NULL;
```

**Cost**: ai_gen() and ai_query() incur Foundation Model API token costs.
Estimate: ~100 tokens per generated field value. Budget accordingly.
