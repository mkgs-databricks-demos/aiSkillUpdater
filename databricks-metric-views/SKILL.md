...
|-------|------|-------------|
| YAML Syntax | [references/yaml-reference.md](references/yaml-reference.md) | Complete YAML spec: fields, measures, joins, materialization |
...
To migrate a regular view to a metric view, treat its `SELECT` source as the metric view's `source`, then promote `GROUP BY` columns to `fields` and aggregations to `measures`. The new metric view does not replace the original — it sits alongside it as a governed metric layer.
...
materialization:                # Optional (Preview)
...
> **Note:** The `fields:` keyword is the canonical name for what were previously called `dimensions`. The `dimensions:` keyword is still accepted as a legacy alias, but `fields:` should be used in all new metric view definitions.
...
Materialization pre-computes metric aggregations for faster query performance and comes in two types:

- **aggregated**: pre-aggregates metric values for faster queries on known dimension/measure combinations
- **unaggregated**: materializes raw rows (source + joins + filter) for flexible downstream aggregation

Materialization uses Lakeflow Spark Declarative Pipelines under the hood and requires serverless compute enabled. This feature is in **Preview**.

## Common Issues

| Issue | Solution |
|-------|----------|
| **SELECT * not supported** | Must explicitly list fields and use MEASURE() for measures |
| **"Cannot resolve column"** | Field/measure names with spaces need backtick quoting |
...
| **Materialization not working** | Requires serverless compute enabled; currently in Preview |
