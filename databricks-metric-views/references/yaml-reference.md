...
| `fields` | Yes | list | Array of field (dimension) definitions (at least one). `dimensions:` is accepted as a legacy alias. |
| `measures` | Yes | list | Array of measure definitions (at least one). |
| `joins` | No | list | Star/snowflake schema join definitions. |
| `materialization` | No | object | Pre-computation configuration (Preview). |

## Fields (Dimensions)

Fields define the categorical attributes used to group and filter data.
...
fields:
  ...
  - name: Order Year
    expr: EXTRACT(YEAR FROM `Order Month`)  # Can reference other fields
...
- Can reference source columns, SQL functions, CASE expressions, and other fields
...
Fields support optional semantic metadata for BI tool integration and natural language search.

```yaml
# Semantic metadata (optional per field)
fields:
  - name: revenue
    display_name: "Total Revenue"        # Human-readable name for BI tools
    synonyms: ["income", "sales", "rev"] # Alternative names for NL search
    formatting:
      type: currency
      currency_code: USD
      decimal_places: 2
...
|--------------|-------------|
| `display_name` | Human-readable label shown in BI tools and Genie |
| `synonyms` | Alternative names used by natural language search to resolve field references |
| `formatting` | Display formatting hints for BI tools (type, currency_code, decimal_places, etc.) |
...
|------|-------------|-------------|
| `unaggregated` | Materializes full data model (source + joins + filter) as raw rows for flexible downstream aggregation | Expensive source views or many joins |
| `aggregated` | Pre-aggregates metric values for specific field/measure combinations for faster queries | Frequently queried combinations |
