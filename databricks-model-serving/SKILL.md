...
## Query with DatabricksOpenAI Client (Recommended)
...
```python
from databricks.openai import DatabricksOpenAI

# Auto-discovers workspace from environment (DATABRICKS_HOST + DATABRICKS_TOKEN)
# or from the active Databricks CLI profile
...
> **DEPRECATED Feb 2026**: `workspace_client.serving_endpoints.get_open_ai_client()` is deprecated. Use `databricks-openai` with `DatabricksOpenAI()` instead (shown above).

## Query with LangChain Client
...
llm = ChatDatabricks(endpoint="databricks-claude-sonnet-5")

response = llm.invoke("Explain what a Delta table is.")
...
result = structured_llm.invoke("Summarize Delta Lake in 3 points")
```

> **DEPRECATED Feb 2026**: `workspace_client.serving_endpoints.get_langchain_chat_open_ai_client()` is deprecated. Use `databricks-langchain` with `ChatDatabricks` instead (shown above).
...
**Current FM API endpoint roster (July 2026)** — these change frequently, always verify at runtime:

| Endpoint | Type | Notes |
|----------|------|-------|
| `databricks-gpt-5-5-pro` | Chat | GPT-5.5 Pro |
| `databricks-claude-sonnet-5` | Chat | Claude Sonnet 5 |
| `databricks-claude-fable-5` | Chat | Claude Fable 5 (fast) |
| `databricks-llama-4-maverick` | Chat | Llama 4 Maverick (multimodal) |
| `databricks-gemini-3-5-flash` | Chat | Gemini 3.5 Flash |
| `databricks-gemini-3-1-flash` | Chat | Gemini 3.1 Flash |
| `databricks-meta-llama-3-3-70b-instruct` | Chat | Llama 3.3 70B |
| `databricks-qwen3-embedding-0-6b` | Embeddings | Current default embedding model |

> **RETIRED as of July 2026**: `claude-3.7-sonnet`, `gemini-3-pro`, `llama-3.1-405b`, `llama-3`, `DBRX`.

> **IMPORTANT**: Model names change with each release. **Never hardcode endpoint names in production code** — load from config (environment variables, secrets, or a config file) so they can be updated without redeployment.

**Defaults when the user doesn't specify**: pick the highest-numbered Claude Sonnet for agents, `databricks-qwen3-embedding-0-6b` for embeddings — resolve actual names from the live list above.

## AI Functions Optimized Models Mode

`ai_query()` in DBSQL supports an **AI Functions Optimized Models** batch inference mode that uses pre-warmed capacity, distinct from pay-per-token and provisioned throughput.

- **Pre-warmed capacity**: Avoids cold-start latency for batch AI workloads
- **Use case**: High-throughput batch scoring with `ai_query()` in SQL pipelines, notebooks, or jobs
- **Not for**: Real-time serving (use pay-per-token or provisioned throughput instead)

```sql
-- ai_query() automatically uses optimized models mode when available
SELECT
  customer_id,
  ai_query(
...
For full `ai_query()` documentation, see the **`databricks-dbsql`** or **`databricks-ai-functions`** skills.

## Prompt Caching

Prompt caching is supported for Databricks-hosted Claude models (`databricks-claude-*` endpoints). When the same prompt prefix is sent repeatedly, the cached portion is not re-processed, reducing latency and token costs.

- Caching activates automatically when the same prefix (system prompt, large context, few-shot examples) is repeated across requests
- Most effective when the static prefix is large (≥1024 tokens) and requests are sent within the cache TTL
- No code changes required — Databricks handles caching transparently
...
| `ImportError: get_open_ai_client` | This method was deprecated Feb 2026. Install `databricks-openai` and use `DatabricksOpenAI()` instead. |
| `ImportError: get_langchain_chat_open_ai_client` | This method was deprecated Feb 2026. Install `databricks-langchain` and use `ChatDatabricks(endpoint=...)` instead. |
