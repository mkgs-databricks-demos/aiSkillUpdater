...
- `DATABRICKS_ENDPOINT` — model endpoint name (e.g. `databricks-claude-sonnet-5`). Run `databricks serving-endpoints list --profile <PROFILE>` to see available models. **Never hardcode endpoint names** — load from config so they can be updated when the model roster changes.
...
export async function generateEmbedding(text: string): Promise<number[]> {
  // Load endpoint from config — never hardcode
  const endpoint =
    process.env.DATABRICKS_EMBEDDING_ENDPOINT || "databricks-qwen3-embedding-0-6b";
...
Current embedding endpoints: `databricks-qwen3-embedding-0-6b` (1024d, production default). Legacy: `databricks-gte-large-en` (1024d), `databricks-bge-large-en` (1024d) — still available but `qwen3-embedding-0-6b` is recommended for new projects. Set `DATABRICKS_EMBEDDING_ENDPOINT` in `.env` and `app.yaml`.
...
| Endpoint name not found | Model roster changed | Run `databricks serving-endpoints list` to get current names; update config |
